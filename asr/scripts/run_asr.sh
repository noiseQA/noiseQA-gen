...
if [ $# != 1 ]; then
    ...
    echo "Usage: apps/run_asr.sh [options] <text>"
    exit 1
fi
...
data_name=$(basename $ASR_FILES _wavs)

docker run --rm ${vols} espnet/espnet:${container_tag} bash -c "echo $PWD
                                cd /espnet/egs/tedlium2/asr1;
                                mkdir -p data;
                                sed -e 's,^,/,' /noiseqa/${ASR_FILES} > data/${data_name};
                                /espnet/utils/recog_wavs.sh data/${data_name}"
